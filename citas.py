...
def verificacionCita():
    ...
    with open(r'C:\Users\jesushev\Documents\QV\USUARIO.csv') as credentials:
        gen_reader = csv.reader(credentials, delimiter = ',')
        next(gen_reader, None) #Skips headers
        for row in gen_reader:
            account_name = row[0]
    ...
    for i in range(len(master_citas)): #generate a new table with the one that matched then use that table on the other comparision
        ...
        if (len(tableTemp)==1):
            ...
            if (not tableTemp['LATE DELIVERY DATE'].values[0] == str(master_citas[i][1]) ):
                ...
                inconsistencias +="SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVERY DATE ON SYSTEM: {2} LATE DELIVERY DATE ON PORTAL: {3} TIPO VIAJE: {4} CUENTA: {5} CONFIRMATION: {6}".format(tableTemp['SHIPMENT_XID'].values[0],tableTemp['DESTINO FINAL'].values[0],tableTemp['LATE DELIVERY DATE'].values[0],master_citas[i][1],tableTemp['TIPO VIAJE'].values[0],tableTemp['CUENTA'].values[0],tableTemp['CONFIRMATION'].values[0])
                inconsistencias +="\n"            
            else:
            ...
            for j in range(len(tableTemp)-1):
                if(not tableTemp['LATE DELIVERY DATE'].values[j]=="nan"):
                    inconsistencias +="Orden sin LATE DELIVERY DATE\n"
                    inconsistencias +="SHIPMENT_XID: {0} CUENTA: {1} CONFIRMATION:\
                         {2}".format(tableTemp['SHIPMENT_XID'].values[j],tableTemp['CUENTA'].values[j],tableTemp['CONFIRMATION'].values[j])
                    inconsistencias +="\n"
                else:
                    if (not tableTemp['LATE DELIVERY DATE'].values[j] == str(master_citas[i][1]) ):
                        print("Dates different")
                        inconsistencias +="SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVERY DATE ON SYSTEM: \
                            {2} LATE DELIVERY DATE ON PORTAL: {3} TIPO VIAJE: {4} CUENTA: {5} CONFIRMATION: {6}".format(tableTemp['SHIPMENT_XID'].values[j],tableTemp['DESTINO FINAL'].values[j],tableTemp['LATE DELIVERY DATE'].values[j],master_citas[i][1],tableTemp['TIPO VIAJE'].values[j],tableTemp['CUENTA'].values[j],tableTemp['CONFIRMATION'].values[j])
                        inconsistencias +="\n"            
                    else:
                        print("Everything in order, dates equal")
            #inconsistencias +="SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVER DATE ON SYSTEM: {2} TIPO VIAJE: {3} CUENTA: {4} CONFIRMATION: {5}\n".format(tableTemp['SHIPMENT_XID'][i],tableTemp['DESTINO FINAL'][i],tableTemp['LATE DELIVERY DATE'][i],tableTemp['TIPO VIAJE'][i],tableTemp['CUENTA'][i],tableTemp['CONFIRMATION'][i])
            # for j in range(len(tableTemp)):
            #     if(not tableTemp.empty):
            #         #print("PRINTING")
            #         #arrTemp= pandas.Series(tableTemp)
            #         print("Test",tableTemp.iloc[:i])
                #     tableTemp['LATE DELIVERY DATE'][j] = datetime.strptime(tableTemp['LATE DELIVERY DATE'][j],'%d-%m-%y %H:%M')
                #     tableTemp['LATE DELIVERY DATE'][j]= str(pandas.to_datetime(tableTemp['LATE DELIVERY DATE'][j])-timedelta(days=1))
                #     #print(tableTemp['LATE DELIVERY DATE'])
                #     #print(master_citas[i][1])
                #     #date= datetime.strptime(tableTemp['LATE DELIVERY DATE'][i],'%d-%m-%Y %H:%M:%S'
                #     if (not tableTemp['LATE DELIVERY DATE'][j] == str(master_citas[i][1]) ):
                #         print("Dates different")               
                #         inconsistencias +="SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVER DATE ON SYSTEM: {2} TIPO VIAJE: {3} CUENTA: {4} CONFIRMATION: {5}\n".format(tableTemp['SHIPMENT_XID'][i],tableTemp['DESTINO FINAL'][i],tableTemp['LATE DELIVERY DATE'][i],tableTemp['TIPO VIAJE'][i],tableTemp['CUENTA'][i],tableTemp['CONFIRMATION'][i])
            ...
            #     #"LATE DELIVERY DATE INCORRECT",tableTemp['LATE DELIVERY DATE'],"TIPO VIAJE",tableTemp["TIPO VIAJE"],"CUENTA",tableTemp["CUENTA"],"CONFIRMATION",tableTemp["CONFIRMATION"])
                ...
                #print("NOT ENTERING")
        # elif(len(tableTemp)==1):
        #     print(len(tableTemp),"LENGHT==0")
        #     print(tableTemp)
        #     if(not tableTemp.empty):
        #         print("IN")
        #         print("Test",tableTemp['LATE DELIVERY DATE'])
        #         tableTemp['LATE DELIVERY DATE'] = datetime.strptime(tableTemp['LATE DELIVERY DATE'],'%d-%m-%y %H:%M')
        #         tableTemp['LATE DELIVERY DATE']= str(pandas.to_datetime(tableTemp['LATE DELIVERY DATE'])-timedelta(days=1))
        #         print(tableTemp['LATE DELIVERY DATE'])
        #         print(master_citas[i][1])
        #         #date= datetime.strptime(tableTemp['LATE DELIVERY DATE'][i],'%d-%m-%Y %H:%M:%S'
        #         if (not tableTemp['LATE DELIVERY DATE'] == str(master_citas[i][1]) ):
        #             print("Dates different") 
        #             inconsistencias = inconsistencias +"SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVER DATE ON SYSTEM: {2} TIPO VIAJE: {3} CUENTA: {4} CONFIRMATION: {5}\n".format(tableTemp['SHIPMENT_XID'][i],tableTemp['DESTINO FINAL'][i],tableTemp['LATE DELIVERY DATE'][i],tableTemp['TIPO VIAJE'][i],tableTemp['CUENTA'][i],tableTemp['CONFIRMATION'][i])
        #                         #"<html><body><h1>Inconsistencia en LATE DELIVERY DATE</h1><p>\nSHIPMENT_XID: {0}</p></body></html>".format(tableTemp['SHIPMENT_XID'][i])
        #     #     #reportFile.write("Inconsistencia en LATE DELIVERY DATE\n") 
        #     #     #reportFile.write("Late Delivery Date\n")
        #     #     #reportFile.write("SHIPMENT_XID: {0} DESTINO FINAL: {1} LATE DELIVER DATE ON SYSTEM: {2} TIPO VIAJE: {3} CUENTA: {4} CONFIRMATION: {5}".format(tableTemp['SHIPMENT_XID'][i],
    ...
    if (not inconsistencias== ""):
        ...
        sendEmail("A01377358@itesm.mx",body+inconsistencias,"Reporte Inconsistencias")
...
#schedule.every().day.at("07:00").do(verificacionCita)
#schedule.every().day.at("10:00").do(verificacionCita)
#schedule.every().day.at("13:00").do(verificacionCita)
#schedule.every().day.at("16:00").do(verificacionCita)
#schedule.every().day.at("19:00").do(verificacionCita)
#schedule.every().day.at("22:00").do(verificacionCita)
#schedule.every().day.at("01:00").do(verificacionCita)
#schedule.every().day.at("04:00").do(verificacionCita)
